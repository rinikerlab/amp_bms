...
def get_potential_energies(pdb_path:str, dcd_path:str, simulation, lambdas_lj:list[float], lambdas_coulomb:list[float]):
    ...
    for frame in tqdm.tqdm(traj):
        ...
        energy_per_lambda_AMP = []
        energy_per_lambda_LJ = []
        ...
        for lambda_lj, lambda_coulomb in zip(lambdas_lj, lambdas_coulomb):

            simulation.context.setParameter('scaling_lj_qm_mm', lambda_lj)
            simulation.context.setParameter('scaling_factor_alchemical_coulomb', lambda_coulomb)
            ...
            energy_per_lambda_AMP.append(U_AMP)
            energy_per_lambda_LJ.append(U_LJ)

        energies_AMP.append(energy_per_lambda_AMP)
        energies_LJ.append(energy_per_lambda_LJ)
...
def main():
    ...
    energies_AMP, energies_LJ = get_potential_energies(pdb_path=parameters["pdb_path"],
                                      ...
                                      lambdas_coulomb=parameters["lambdas_coulomb"],
                                      lambdas_lj=parameters["lambdas_lj"])
