...
lambda_coulomb = 1.0 # still need to set it to match the equilibration run 
lambda_lj = 1.0 # still need to set it to match the equilibration run
mol_i = 12
...
for key in range(len(lambda_pol)):
    ...
    write_alchemical_FE_json(alchemical_json_path, lambda_coulomb=lambda_coulomb, lambda_lj=lambda_lj)
    params = load_yaml(amp_parameters_path)
    params["pol_scaling"] = float(lambda_pol[key])
    amp_parameters_path_new = os.path.join(dir_path, "amp_parameters.yaml")

    with open(amp_parameters_path_new, 'w') as file:
        yaml.dump(params, file)
        
    #write parameters YAML file, adjust the path as necessary
    command = f"""python {write_yaml_py} {yaml_path} {dir_path} {f"lambda_{key:03d}"} {pdb_path} {molecules_path} \
                    ...
                    --save_final_state --out_state_xml_path {xml_save_path} --forcefield {forcefield_path} --not_minimize \
                    ...
                    --alchemical_FE --alchemical_FE_definition {alchemical_json_path} \
                    --tip4p --continue_simulation --in_state_xml_path {in_xml_path} --ff_name {ff_name}
                    """
